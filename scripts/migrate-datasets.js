...
async function saveDatapackageRaw(dataset) {
 ...
 const dsName = dataset.name;
 const dir = path.join(DATA_DIR, org, dsName);
 const dpPath = path.join(dir, 'datapackage.json');
 if (fs.existsSync(dpPath)) {
   console.log(`Skip (already exists): ${org}/${dsName}/datapackage.json`);
   return;
 }
 fs.mkdirSync(dir, { recursive: true });
 fs.writeFileSync(dpPath, JSON.stringify(dataset, null, 2));
...
async function main() {
 ...
 for (const dsName of datasetList) {
   try {
     const dataset = await retry(() => fetchDataset(dsName), 3, 2000);
     await retry(() => saveDatapackageRaw(dataset), 3, 2000);
   } catch (e) {
